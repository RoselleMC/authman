...
var defs = []Def{
	...
	{Key: "gate.kick.downstream_unavailable", Default: "<red>The downstream server is currently unavailable.<newline>目标下游服务器当前不可用。</red><newline><gray>Please try {server} again later.</gray>", Placeholders: []string{"player", "server"}, Scope: ScopeGate},
